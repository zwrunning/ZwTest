...
public class DictModelManager {
    ...
    // 全局唯一访问入口
    static let shareManager = DictModelManager()
                ...
                if let value: AnyObject = dict[k] {
                    ...
                    if v.isEmpty {
                        ...
                        if type == "NSDictionary" {
                            ...
                            if let subObj: AnyObject = objectWithArray(value as! NSArray, cls: NSClassFromString(ns + "." + v)!) {
                                obj.setValue(subObj, forKey: k)
                            }
                        }
                    }
    ...
    ///
    ///  - returns: 模型数组
    public func objectWithArray(array: NSArray, cls: AnyClass) -> NSArray? {
